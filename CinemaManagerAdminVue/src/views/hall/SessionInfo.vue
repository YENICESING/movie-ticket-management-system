<!--数据获取方法
getSessionList：根据查询条件向服务器发送请求，获取场次列表数据。
getHallList：向服务器发送请求，获取影厅列表数据。
getMovieList：向服务器发送请求，获取电影列表数据。
分页处理方法
handleSizeChange：当用户更改每页显示的记录数量时，更新查询条件中的 pageSize，并重新调用 getSessionList 方法获取数据。
handleCurrentChange：当用户切换页码时，更新查询条件中的 pageNum，并重新调用 getSessionList 方法获取数据。
对话框操作方法
showAddDialog：显示添加场次对话框。
addSession：处理添加场次的逻辑，包括表单验证、计算结束时间和截止时间、获取影厅座位信息、发送添加请求等。
isAbleEdit：检查场次是否有相关订单，若没有则调用 showEditDialog 方法显示修改对话框；若有则弹出提示框。
showEditDialog：根据场次 ID 获取场次详细信息，并显示修改对话框。
editSessionInfo：处理修改场次信息的逻辑，包括表单验证、计算结束时间和截止时间、发送更新请求等。
checkSeat：根据场次 ID 获取场次的座位信息，并显示查看座位对话框。
删除操作方法
isAbleMultipleDelete：检查批量删除的场次是否有相关订单，若没有则调用 multipleDelete 方法进行批量删除；若有则弹出提示框。
multipleDelete：处理批量删除场次的逻辑，包括确认删除提示、发送删除请求等。
isAbleDelete：检查单个场次是否可以删除，若场次已结束或没有相关订单则调用 deleteSessionById 方法进行删除；若有相关订单则弹出提示框。
deleteSessionById：处理单个场次删除的逻辑，包括确认删除提示、发送删除请求等。
辅助方法
handleEndTimeAndDeadline：根据电影时长和播放时间计算场次的结束时间和截止时间。
handleSelectionChange：监听表格中多选框的选择变化，更新选中的场次列表。-->
<template>
  <div>
    <!--面包屑导航区域-->
    <div class="board">
