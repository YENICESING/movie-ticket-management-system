<!--数据管理
订单信息：通过 billId 从路由参数获取当前订单 ID，billInfo 存储订单详细信息，billSeats 存储所选座位，payState 和 cancelState 分别记录支付和取消状态，
cancelTime 记录取消时间。
倒计时数据：minutes 和 seconds 用于记录支付倒计时的分钟和秒数。
生命周期钩子
created：组件创建后调用 getBillInfo 方法，获取订单详细信息，检查用户操作合法性，并计算支付剩余时间。
mounted：组件挂载后调用 add 方法，启动支付倒计时。
方法实现
获取订单信息：getBillInfo 方法通过 API 请求获取订单详情，验证用户身份，解析座位信息，更新支付和取消状态，并计算支付剩余时间。
订单支付：payForBill 方法将订单支付状态更新为已支付，发送 PUT 请求更新订单信息，根据响应结果提示用户支付成功或失败，并进行页面跳转。
订单取消：cancelForBill 方法将订单取消状态更新为已取消，记录取消时间，恢复所选座位状态，发送 PUT 请求更新订单和座位信息，根据响应结果提示用户取消成功或失败，
并进行页面跳转。
计算剩余时间：computeLeftTime 方法根据订单创建时间和当前时间计算支付剩余时间，并更新倒计时数据。
数字格式化：num 方法将数字格式化为两位数的字符串，用于倒计时显示。
启动倒计时：add 方法使用 setInterval 每秒更新一次倒计时数据，倒计时结束时清除定时器。
监听器和计算属性
监听器：监听 second 和 minute 的变化，调用 num 方法进行格式化。
计算属性：second 和 minute 计算属性用于格式化倒计时的显示。-->
<template>
  <div>
