<!--数据获取方法
getBillList：根据查询条件向服务器发送请求，获取订单列表数据，并更新页面显示。
分页处理方法
handleSizeChange：当用户更改每页显示的记录数量时，更新查询条件中的 pageSize，并重新调用 getBillList 方法获取数据。
handleCurrentChange：当用户切换页码时，更新查询条件中的 pageNum，并重新调用 getBillList 方法获取数据。
多选框处理方法
handleSelectionChange：监听表格中多选框的选择变化，更新 multipleSelection 数组，用于批量删除操作。
编辑操作方法
showEditDialog：根据订单 ID 获取订单详细信息，若订单已完成或已取消则弹出提示框，否则显示修改订单对话框。
editDialogClosed：在修改订单对话框关闭时，重置表单字段。
editBillInfo：处理订单信息的修改逻辑，根据用户选择的支付状态和取消状态进行不同的处理，如支付订单、取消订单，并更新相应的订单和座位信息。
删除操作方法
multipleDelete：处理批量删除订单的逻辑，先弹出确认提示框，确认后将选中的多个订单信息删除。
deleteBillById：处理单个订单删除的逻辑，先弹出确认提示框，确认后根据订单 ID 删除订单信息。-->
<template>
  <div>
    <!--面包屑导航区域-->
    <div class="board">
