<!--这段代码是一个 Vue.js 组件，主要用于影厅信息的管理，包括影厅信息的搜索、添加、编辑、删除以及座位安排等功能
数据获取
getHallList()：获取影厅列表数据。
getHallCategoryList()：获取影厅类别列表。
分页处理
handleSizeChange(newSize)：处理每页显示数量的更改。
handleCurrentChange(newPage)：处理页码切换。
对话框操作
addHall()：添加影厅信息。
editHallInfo()：编辑影厅信息。
arrangeSeat(id)：展示座位安排对话框。
saveSeat()：保存座位安排。
删除操作
isAbleDelete(id) 和 isAbleMultipleDelete()：检查是否可删除。
deleteHallById(id) 和 multipleDelete()：执行删除操作。-->
