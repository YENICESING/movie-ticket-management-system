...
///SysSession 类是一个 Java 实体类，用于表示场次信息，实现了 Serializable 接口，表明该类的对象可以被序列化，适用于网络传输或存储操作。
// 该类包含了场次的各种属性，如场次编号、影厅编号、语言版本、电影编号、播放时间、结束时间、截止时间、场次日期、场次票价、场次提示、座位信息、座位数、
// 已售座位数以及关联的影厅和电影信息，并提供了相应的属性访问器（getter）和修改器（setter）方法，同时重写了 equals、hashCode 和 toString 方法，
// 方便对象的比较和信息输出。
/**
 * 场次实体类
 */
