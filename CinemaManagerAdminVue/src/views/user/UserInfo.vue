<!--数据获取方法
getUserList：根据查询条件向服务器发送请求，获取用户列表数据，并更新页面显示。
getRoleList：向服务器发送请求，获取角色列表数据，用于在添加和修改用户时选择权限。
分页处理方法
handleSizeChange：当用户更改每页显示的记录数量时，更新查询条件中的 pageSize，并重新调用 getUserList 方法获取数据。
handleCurrentChange：当用户切换页码时，更新查询条件中的 pageNum，并重新调用 getUserList 方法获取数据。
对话框操作方法
showAddDialog：显示添加用户对话框。
addDialogClosed：在添加用户对话框关闭时，重置表单字段和图片列表。
addUser：处理添加用户的逻辑，包括上传图片、表单验证、发送添加请求等。
showEditDialog：根据用户 ID 获取用户详细信息，并显示修改用户对话框，同时回显用户的头像信息。
editDialogClosed：在修改用户对话框关闭时，重置表单字段和图片列表。
cancelEdit：取消修改操作，关闭对话框并清空删除图片列表。
editUserInfo：处理修改用户信息的逻辑，包括上传图片、表单验证、发送更新请求，若修改成功则删除之前删除的图片。
删除操作方法
multipleDelete：处理批量删除用户的逻辑，先弹出确认删除的提示框，若用户确认则发送删除请求。
deleteUserById：处理单个用户删除的逻辑，先弹出确认删除的提示框，若用户确认则发送删除请求。
图片处理方法
handleRemove：处理删除图片的逻辑，将已上传的图片添加到 deletePicList 数组中，并从图片列表中移除。
handlePictureCardPreview：处理图片预览的逻辑，点击图片时弹出放大预览对话框。
handleChange：处理图片选择变化的逻辑，更新图片列表。
handleSuccess：处理图片上传成功的逻辑，将上传成功的图片信息添加到 pictureList 数组中。
handleError：处理图片上传失败的逻辑，打印错误信息。
handleExceed：处理图片上传数量超过限制的逻辑，弹出错误提示框。
submitFile：处理图片上传的逻辑，将未上传的图片发送到服务器，并将上传成功的图片信息添加到 pictureList 数组中。
辅助方法
sexFormat：用于格式化用户性别信息，将布尔值转换为 “男” 或 “女” 显示在表格中。-->
<template>
  <div>
    <!--面包屑导航区域-->
    <div class="board">
