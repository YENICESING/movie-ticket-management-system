<!--文件上传
使用 el-upload 组件实现文件上传功能，支持选择文件、上传、删除等操作。
handleChange 方法处理文件选择变化，更新 pics 数组并显示图片对话框。
handleSuccess 方法处理文件上传成功后的逻辑，更新 pics 数组并调用 submitFile 方法进行文件上传操作。
handleError 方法处理文件上传失败的逻辑，将错误信息打印到控制台。
handleExceed 方法处理文件选择数量超过限制的逻辑，显示错误信息提示用户。
submitFile 方法遍历 pics 数组，将未上传的文件通过 axios 发送 POST 请求上传到服务器，并将上传成功后的文件路径添加到 pictureList 数组中。
5. 文件删除
handleRemove 方法处理删除文件的逻辑，从 pics 数组中移除指定文件，并将已成功上传的文件路径添加到 deletePicList 数组中。
6. 表单验证
使用 el-form 和 el-form-item 组件创建表单，并通过 loginUserRules 定义表单验证规则。
定义了 checkEmail 和 checkMobile 两个验证函数，分别用于验证电子邮箱和手机号码的合法性。-->
<template>
